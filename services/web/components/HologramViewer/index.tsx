...
const HologramViewer: React.FC<HologramViewerProps> = ({ avatars: avatarsProp, visemeStates }) => {
  ...
  const buildAvatarGroup = useCallback(
    (avatar: AvatarData): THREE.Group => {
      ...
      // ── Silhouette contour lines ─────────────────────────
      // Trace the outline of the body with glowing connected lines
      {
        const contourColor = new THREE.Color(HOLOGRAM_COLOR);
        ...
        // Group particles into Y bands and find left/right silhouette
        const bandSize = 0.025;
        ...
        for (let y = yMin; y <= yMax; y += bandSize) {
          const band = worldPts.filter((p) => p.y >= y && p.y < y + bandSize);
          if (band.length < 3) continue;

          // Find leftmost and rightmost particles (front-facing: Z > -0.01)
          const frontBand = band.filter((p) => p.z > -0.01);
          if (frontBand.length < 2) continue;

          let minX = Infinity, maxX = -Infinity;
          let leftPt: WorldPt | null = null, rightPt: WorldPt | null = null;
          for (const p of frontBand) {
            if (p.x < minX) { minX = p.x; leftPt = p; }
            if (p.x > maxX) { maxX = p.x; rightPt = p; }
          }

          const bandY = y + bandSize / 2;
          if (leftPt) leftEdge.push(new THREE.Vector3(leftPt.x, bandY, leftPt.z));
          if (rightPt) rightEdge.push(new THREE.Vector3(rightPt.x, bandY, rightPt.z));
        }

        // Sort by Y
        leftEdge.sort((a, b) => a.y - b.y);
        rightEdge.sort((a, b) => a.y - b.y);

        // Draw left silhouette line
        if (leftEdge.length > 2) {
          const geo = new THREE.BufferGeometry().setFromPoints(leftEdge);
          group.add(new THREE.Line(geo, contourMat));
        }
        // Draw right silhouette line
        if (rightEdge.length > 2) {
          const geo = new THREE.BufferGeometry().setFromPoints(rightEdge);
          group.add(new THREE.Line(geo, contourMat));
        }

        // Horizontal contour rings at key Y levels
        const ringYLevels = [0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80,
                             -0.10, -0.20, -0.30, -0.40, -0.50, -0.60, -0.70, -0.80];
        for (const ringY of ringYLevels) {
          const band = worldPts.filter((p) => Math.abs(p.y - ringY) < bandSize);
          if (band.length < 4) continue;

          // Sort by angle around center
          const cx = band.reduce((s, p) => s + p.x, 0) / band.length;
          const cz = band.reduce((s, p) => s + p.z, 0) / band.length;
          const sorted = band
            .map((p) => ({ ...p, angle: Math.atan2(p.z - cz, p.x - cx) }))
            .sort((a, b) => a.angle - b.angle);

          // Find outermost point per angular segment (8 segments)
          const ringPts: THREE.Vector3[] = [];
          const segments = 12;
          for (let s = 0; s < segments; s++) {
            const aMin = (s / segments) * Math.PI * 2 - Math.PI;
            const aMax = ((s + 1) / segments) * Math.PI * 2 - Math.PI;
            const seg = sorted.filter((p) => p.angle >= aMin && p.angle < aMax);
            ...
            ringPts.push(new THREE.Vector3(best.x, ringY, best.z));
          }

          if (ringPts.length > 3) {
            ringPts.push(ringPts[0].clone()); // close the ring
